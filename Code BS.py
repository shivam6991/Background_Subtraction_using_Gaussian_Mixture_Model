...
# For each pixel, we have taken K=3 Gaussian distributions for each of Red, Blue and Green color values.

mr=np.zeros((w,h,3),dtype=np.float64,order='C')
v=np.zeros((w,h,3),dtype=np.float64,order='C') # Variance of each Gaussian
...
while count<=997:
    ...
    #cv2.imshow('fg',fg)
    cv2.imwrite( r"bg8.2\frame%d.jpg" %count, bg );
    med = cv2.medianBlur(fg,3)
    cv2.imwrite( r"fg8.2\frame%d.jpg" %count, med );
    count += 1
